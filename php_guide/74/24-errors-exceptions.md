...
# Erros e Exceções no PHP 7.4

Este documento cobre o tratamento de erros e exceções no PHP 7.4. Todo o código
aqui é 100% válido e executável na versão 7.4.

> Aviso de escopo: nada aqui é específico de versões 8.x. Veja também
> `06-logs.md` e `17-logs-74.md`.
...
No PHP 7.4, `JsonException` também estende `Exception` (lançada por `json_encode`/
`json_decode` quando `JSON_THROW_ON_ERROR` é usado — recurso do 7.3):
...
| `JsonException` | 7.3 | `JSON_THROW_ON_ERROR` |
