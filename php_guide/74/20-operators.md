...
# Operadores e Sintaxe Básica no PHP 7.4

Este documento cobre operadores e lembretes de sintaxe disponíveis no PHP 7.4.
Todo o código aqui é 100% válido e executável na versão 7.4.

> Aviso de escopo: nada aqui é específico de versões 8.x. A coalescência nula
> `??`, o operador nave `<=>` e a atribuição de coalescência `??=` pertencem ao
> 7.0/7.4 e, portanto, são válidos nesta versão.
...
### Atribuição de coalescência nula `??=` (7.4)

Atribui o lado direito **apenas se** a variável/índice não estiver definido(a)
ou for `null`.

```php
<?php
$config = [];
$config['timeout'] ??= 30; // define 30 porque não existe
$config['timeout'] ??= 60; // mantém 30 (já definido)
...
## 12. Operadores de tipo (`instanceof`)
...
## 15. Armadilhas de comparação (`==` vs `===`)
...
## 17. Precedência de operadores (resumo)
